...
max_y_value = max(max(y_plot[mask]), max(data_lna_new["X_part"]))
min_y_value = min(min(y_plot[mask]), min(data_lna_new["X_part"]))
y_lower = min_y_value - abs(min_y_value) * 0.003
y_upper = max_y_value + abs(max_y_value) * 0.005
