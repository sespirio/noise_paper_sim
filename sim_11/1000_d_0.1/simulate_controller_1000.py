...
add_event(name=f'parm_k_in', trigger='{Time}=={15}',
          assignments=[[f'Values[k_in]', '{Values[k_in].InitialValue}+1.6']])
...
result_det = run_time_course(duration=t_f, automatic=False, method="deterministic", max_steps=10000000, stepsize=0.1,
                         use_numbers=True)
...
for k in range(runs_stochastic):
    result = run_time_course(duration = t_f, automatic = False, method = method_sim, max_steps = 100000000,
