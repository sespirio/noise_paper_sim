...
mask = t_plot >= t_final
line3, = ax1.plot(t_plot[mask] - t_final, du_dt_gill[mask], linestyle="dashdot", color="#D55E00", label = "$\mathrm{d}U / \mathrm{d}t \mathrm{, \, SSA}$", alpha = 0.8)
...
FF_x = FF_X_star = 1 + (
    params_new["k_3"] * (
        ...
        + params_new["c"] * params_new["k_in"] * params_new["p_1"] * (
            2 * params_new["k_2"] / params_new["p_2"]
            + (
                (1 + params_new["c"]) * (params_new["k_3"] ** 2) * params_new["k_in"]
            ) / (
                params_new["b"] * params_new["k_2"] * params_new["p_2"]
                + params_new["k_3"] * (
                    params_new["k_1"] * params_new["k_3"] + (params_new["p_2"] ** 2)
                )
            )
        )
    )
) / (
    2 * params_new["b"] * (params_new["k_2"] ** 2)
)
...
y_lower = min_y_value - abs(min_y_value) * 0.06
y_upper = max_y_value + abs(max_y_value) * 0.06
plt.ylim(y_lower, y_upper)
