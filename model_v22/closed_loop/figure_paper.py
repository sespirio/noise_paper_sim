...
line1, = ax1.plot(t_mean_controller, mean_controller_, linestyle="solid", color='green', label = "$X\mathrm{, \, SSA, \, CL}$", alpha = 0.7)

line2, = ax1.plot(data_lna["t"], data_lna["Y_part"], linestyle="dashed", color='black', label = "$X\mathrm{, \, LNA, \, CL}$", alpha = 0.6)
...
plt.plot(t_index_controller_, fano_controller_, linestyle="solid", color='green', label = "$X\mathrm{, \, SSA, \, CL}$", alpha = 0.7)


data_lna_ = data_lna.dropna()
plt.plot(data_lna_["t"], data_lna_["y_fano_lna"], linestyle="dashed", color='black', label = "$X\mathrm{, \, LNA, \, CL}$", alpha = 0.6)
...
lambda_1 = (
    k_1 * k_3**2 * (
        (-1 + c) * c * k_2**3 * p_1 * (c * k_in * p_1 + b * p_2)
        + k_2 * k_3 * k_f * (
            c * ((-1 + c) * k_2 + 2 * k_f) * k_in * p_1
            + 2 * b * k_f * p_2
        )
        + 2 * k_3**2 * k_f**2 * (k_f * k_in + p_2**2)
    )
    + k_2 * (
        2 * k_3**3 * k_f**3 * k_in * (k_in + p_2)
        + (-1 + c) * c * k_2**3 * p_1 * (c * k_in * p_1 + b * p_2)**2
        + k_2 * k_3**2 * k_f**2 * (
            c * (3 + c) * k_in**2 * p_1
            + 2 * k_in * (b + c * p_1) * p_2
            + 2 * b * p_2**2
        )
        + c * k_2**2 * k_3 * p_1 * (c * k_in * p_1 + b * p_2) * (
            2 * c * k_f * k_in + (-1 + c) * p_2**2
        )
    )
)

lambda_2 = (
    2 * k_2 * (k_3 * k_f + c * k_2 * p_1) * (
        k_1 * k_3**2 * (k_3 * k_f * k_in + c * k_2 * k_in * p_1 + b * k_2 * p_2)
        + k_2 * (c * k_in * p_1 + b * p_2) * (
            k_3 * k_f * k_in
            + c * k_2 * k_in * p_1
            + b * k_2 * p_2
            + k_3 * p_2**2
