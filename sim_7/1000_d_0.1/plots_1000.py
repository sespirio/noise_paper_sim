...
def ode_system(t, y, b, k_1, k_2, k_3, n, d):
...
def solve_and_save(y0, params, t_final, csv_filename):
    ...
    sol = solve_ivp(
        ...
        t_span=t_span,
        y0=y0,
        args=(params['b'], params['k_1'], params['k_2'], params['k_3'], params['n'], params['d']),
        t_eval=t_eval,
        method = 'LSODA'
    )
...
def ode_system_new(t, y, b, k_in, k_1, k_2, k_3, n,
    ...
    dV_UX     = k_in * V_U - p_2 * V_UX - k_1 * X * V_UZ1 - (k_1 * Z_1 + d) * V_UX
    ...
    dV_X      = b + d * X + k_in * U + 2 * k_in * V_UX + k_1 * X * Z_1 - 2 * k_1 * X * V_XZ1 - 2 * (d + k_1 * Z_1) * V_X


    dV_XZ1    = k_2 * V_X + k_in * V_UZ1 - k_1 * X * V_Z1 - (d + k_1 * Z_1) * V_XZ1 - n * Z_1 * V_XZ2 - n * Z_2 * V_XZ1


    dV_XZ2    = k_in * V_UZ2 - k_1 * X * V_Z1Z2 - (d + k_1 * Z_1) * V_XZ2 - n * Z_1 * V_XZ2 - n * Z_2 * V_XZ1
...
def solve_and_save_new(y0_new, params_new, t_final_new, csv_filename):
    ...
    sol = solve_ivp(
        ...
        args=(params_new['b'], params_new['k_in'], params_new['k_1'], params_new['k_2'], params_new['k_3'],
              params_new['n'], params_new['d'], params_new['c'], params_new['p_1'], params_new['p_2']),
        t_eval=t_eval,
        method = 'LSODA'
    )
...
import pickle
with open(folder_path_exp + f'/fano_gillespie_d_{params_new["d"]}.pkl', 'wb') as f:
    pickle.dump({
        "t": t_index_controller_,
        "fano_x": fano_controller_
    }, f)


with open(folder_path_exp + f'/fano_lna_d_{params_new["d"]}.pkl', 'wb') as f:
    pickle.dump({
        "t": list(data_lna_["t"]),
        "fano_x": list(data_lna_["x_fano_lna"])
    }, f)
