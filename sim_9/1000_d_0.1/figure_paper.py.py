...
def ode_system(t, y, b, k_1, k_2, k_3, n, d):
    ...
    dZ_1 = k_2 * X - n * Z_1 * Z_2 - d * Z_1


    dZ_2 = k_3 - n * Z_1 * Z_2 - d * Z_2
...
for plot_i in range(n_plots):
    ...
    for k, trajectory_i in enumerate(sampled_trajectories):
        ...
        plt.plot(t_plot, y_plot, color = colors[k], linestyle="solid", alpha=0.7)



        if max(y_plot)>max_y_temp:
            max_y_temp = float(max(y_plot))
        if min(y_plot)<min_y_temp:
            min_y_temp = float(min(y_plot))
