...
def ode_system_new(t, y, b, k_in, k_1, k_2, k_3, n, c, p_1, p_2):
    ...
    dV_UX    = k_in * V_U     - p_2 * V_UX    - k_1 * X * V_UZ1 - k_1 * Z_1 * V_UX

    dV_UZ1   = k_2 * V_UX     - p_2 * V_UZ1  - n   * Z_1 * V_UZ2 - n * Z_2 * V_UZ1

    dV_UZ2   =  - p_2 * V_UZ2  - n * Z_1 * V_UZ2 - n * Z_2 * V_UZ1

    dV_X     = b + k_in * U + 2 * k_in * V_UX  + k_1 * X * Z_1  - 2 * k_1 * X * V_XZ1 - 2 * k_1 * Z_1 * V_X

    dV_XZ1   = k_2 * V_X  + k_in * V_UZ1  - k_1 * X * V_Z1 - k_1 * Z_1 * V_XZ1  - n * Z_1 * V_XZ2 - n * Z_2 * V_XZ1

    dV_XZ2   = k_in * V_UZ2 - k_1 * X * V_Z1Z2 - k_1 * Z_1 * V_XZ2 - n * Z_1 * V_XZ2 - n * Z_2 * V_XZ1
...
FF_x = FF_X_star = 1 + (
    params_new["k_3"] * params_new["p_2"] * (
        ...
        + params_new["c"] * (1 + params_new["c"]) * (params_new["k_2"]**2) * (params_new["k_in"]**2) * params_new["p_1"]
    ...
    2 * (params_new["k_2"]**2) * (params_new["c"] * params_new["k_in"] * params_new["p_1"] + params_new["b"] * params_new["p_2"])
    * (
