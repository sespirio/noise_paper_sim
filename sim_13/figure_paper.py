...
n_values = [0.1, 1, 10, 100]




with open(f'1000_n_{1000}/stochastic_simulation_paper/fano_gillespie_n_{1000}.pkl', 'rb') as f:
    gill_data = pickle.load(f)
    t_index_controller_0 = gill_data["t"]
    fano_controller_0 = gill_data["fano"]


with open(f'1000_n_{1000}/stochastic_simulation_paper/fano_lna_n_{1000}.pkl', 'rb') as f:
    lna_data = pickle.load(f)
    t_lna_0 = lna_data["t"]
    fano_lna_0 = lna_data["fano"]
...
for idx, n_i in enumerate(n_values):
    ...
    with open(f'1000_n_{n_i}/stochastic_simulation_paper/fano_gillespie_n_{n_i}.pkl', 'rb') as f:
        gill_data = pickle.load(f)
        t_index_controller_ = gill_data["t"]
        fano_controller_ = gill_data["fano"]


    with open(f'1000_n_{n_i}/stochastic_simulation_paper/fano_lna_n_{n_i}.pkl', 'rb') as f:
        lna_data = pickle.load(f)
        t_lna = lna_data["t"]
        fano_lna = lna_data["fano"]
    ...
    plt.savefig(folder_path_exp + "/" + f"x_fano_n_{n_i}.svg", format='svg', dpi=1000)
    plt.savefig(folder_path_exp + "/" + f"x_fano_n_{n_i}.pdf", format='pdf', dpi=1000)
