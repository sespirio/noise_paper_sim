...
plt.plot(t_index_controller, mean_controller, linestyle="solid", color="green", label = "$X\mathrm{, \, SSA}$", alpha = 0.7)

plt.plot(data_lna["t"], data_lna["X_part"], linestyle="dashed", color="black", label = "$X\mathrm{, \, LNA}$", alpha = 0.6)
...
max_y_value = max(max(mean_controller), max(data_lna["X_part"]))
min_y_value = min(min(mean_controller), min(data_lna["X_part"]))
y_lower = min_y_value - abs(min_y_value) * 0.20
y_upper = max_y_value + abs(max_y_value) * 0.10
...
plt.plot(data_lna_["t"], data_lna_["x_fano_lna"], linestyle="dashed", color="black", label = "$X\mathrm{, \, LNA}$", alpha = 0.6)


FF_x = 1 + ((params["k_1"]*params["k_3"]**2)/(params["b"]*params["k_2"]**2))
plt.hlines(FF_x, min(data_lna_["t"]), max(data_lna_["t"]), color="#D55E00", alpha = 0.8, linestyle="dashdot", label = "$FF_X^*$")
...
max_y_value = max(max(fano_controller_), max(data_lna_["x_fano_lna"]))
min_y_value = min(min(fano_controller_), min(data_lna_["x_fano_lna"]))
y_lower = min_y_value - abs(min_y_value) * 0.8
y_upper = max_y_value + abs(max_y_value) * 0.1
plt.ylim(y_lower, y_upper)
...
for plot_i in range(n_plots):
    ...
    y_lower = min_y_temp - abs(min_y_temp) * 0.20
    y_upper = max_y_temp + abs(max_y_temp) * 0.10
